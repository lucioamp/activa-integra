	...
	$(function() {
		modulo_ready = function($this, $oldPage, $mainPage) {
			...
			$this.find('button#alterar').click(function() {
				...
				$this.sendRequest('../../UsuarioServlet?opcao=A',
					{
						pkUsuario: id, nome: nome.val(), apelido: apelido.val(), email: email.val(), dataNasc: dt_nasc.val(), senha: pw_senha.val(),
			...
			adicionarContato = function(id, tipo, tipo_nome, desc) {
				...
				criarBotaoEditar(tdEditar).click(function() { $mainPage.newPage('../../ContatoServlet?opcao=D', {id: id, tipo_contato: tipo, desc: desc, tipo: "paginaAlterar"});  });
			...
			st_usuario.selectOptions("<%= usuario.getStatus() %>".clearNull());
			administrador.attr("checked", "<%= (Administrador.isAdministrador(usuario) ? "checked" : "") %>");
			...
			for(ContatoUsuario contato:contatos)
				out.print("adicionarContato("+contato.getPkContatoUsuario()+", '"+contato.getTipoContato().getPkTipoContato()+"'.clearNull(), '"+contato.getTipoContato().getTipoContato()+"'.clearNull(), '"+contato.getContato()+"'.clearNull());");
			%>
...
<div id="title">Alterar Usu&aacute;rio</div>
