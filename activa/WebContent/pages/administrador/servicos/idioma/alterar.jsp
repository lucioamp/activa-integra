	...
	$(function() {
		modulo_ready = function($this, $oldPage, $mainPage) {
			...
			var botaoVoltar = $this.find('button#voltar').click(function() {
				$this.remove();
				$mainPage.getPage(1).empty().load('../../IdiomaServlet?opcao=C', function() {
			...
			$this.find('button#alterar').click(function() {
				...
				$this.sendRequest('../../IdiomaServlet?opcao=A',
					{pkIdioma: <%= idioma.getPkIdioma() %>, nome: nome.val()},
					function() { botaoVoltar.click(); }
				);
			});

			$this.find('input#nome').val("<%= idioma.getNome() %>".clearNull());
		}
	});
</script>
