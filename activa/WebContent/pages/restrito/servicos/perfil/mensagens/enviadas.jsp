		...
		modulo_ready = function($this) {
			...
			var adicionarMensagem = function(id, nome, assunto, mensagem)
			{
				mostrarMensagem = function(e) {
					e.preventDefault();
					$this.newPage('../../MembroMensagensServlet?opcao=M', {nome: nome, assunto: assunto, mensagem: mensagem, tipo: "Enviada"});
				};
			...
			for(Mensagem mensagem:mensagens)
				out.print("adicionarMensagem("+mensagem.getPkMensagem()+", '"+mensagem.getMembroDestino().getNome()+"'.clearNull(), '"+mensagem.getAssunto()+"'.clearNull(), '"+Ferramenta.clearScape(mensagem.getMensagem())+"'.clearNull());");
			%>
		};
	});
</script>

<div id="title">Mensagens Enviadas</div>
