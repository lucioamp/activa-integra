...
<script type="text/javascript">
	$(function() {
		modulo_ready = function($this) {
			...
			if($_SESSION['topicoTr'] != null)
			{
				object['topicoTr'] = $_SESSION['topicoTr'];
				
				delete $_SESSION['topicoTr'];
				$_SESSION['topicoTr'] = null;
			}
			...
			$this.find('button#alterar').click(function(e) {
				$this.newPage('../../MembroTopicoServlet?opcao=T', {pkTopico: <%= topico.getPkTopico() %>});
			});
			
			$this.find('button#excluir').click(function(e) {
				$this.alert('Tem certeza que deseja excluir este t&oacute;pico?', null, {
					dialog: DIALOG_CONFIRM,
					clickOK: function() {
					$this.sendRequest('../../MembroTopicoServlet?opcao=E',
						{pkTopico: <%= topico.getPkTopico() %>},
						function(data) {
							var result = trim(data);

							if(result == "true")
							{
								$this.alert('O tópico foi excluido com sucesso.', null, {clickOK: function() {
									tab.tabs('remove', object['id']);
									if(object['topicoTr'] != null)
										object['topicoTr'].clear();
								}});
							}else
								$this.alert('Erro ao tentar excluir o tópico');
						}
					);
					}
				});
...
<table class="tabelaResult">
	...
	<tbody>
		...
		<tr>
			...
			<button type="button" class="ui-state-default ui-corner-all"
				name="excluir" id="excluir">Excluir</button>
			</td>
		</tr>
	</tbody>
</table>
