...
public class NoArvoreResposta extends NivelWADL 
{
	...
	public void incluiDB( int id_Resposta, int id_NoPai ) throws ExcecaoBanco
	{
		Iterator it;
		NoArvoreRespostaPersist persist;
		
		setIdNoPai( id_NoPai );
		setIdResposta( id_Resposta );

		persist = new NoArvoreRespostaPersist();
		persist.Inclui(this);
		...
		{
			NoArvoreResposta elem = (NoArvoreResposta)it.next();
			elem.incluiDB(id_Resposta, getIdBanco());
		}
	}
